...
class Game:
    ...
    def __str__(self):
        ...
        if self.missions_lost < 3:
            s = s + "\n[bold bright_green]The Resistance succeeded![/]"
        else:
            s = s + "\n[bold bright_red]The Resistance failed![/]"
        s = s + "\nThe spies were agents: " + str(sorted(self.spies))
        return s
...
class Round:
    ...
    def __str__(self):
        ...
        """
        s = "\n[bold bright_magenta]Round[/]: " + str(self.rnd + 1)
        for m in self.missions:
            s = s + "\n" + str(m)
        if self.is_successful():
            s = s + "\nResistance [bright_green]won[/] the round."
        else:
            s = s + "\nResistance [bright_red]lost[/] the round."
        return s
    ...
    def play(self):
        ...
        while len(self.missions) < 5:
            ...
            if len(set(team)) != mission_size:
                raise Exception(f"Invalid team: {team}")
...
class Mission:
    ...
    def run(self, auto_approve):
        ...
        self.votes_for = [
            ...
            if auto_approve or self.agents[i].vote(self.team, self.leader_id)
        ]
        for a in self.agents:
        ...
        if 2 * len(self.votes_for) > len(self.agents):
            self.fails = [
                i
                for i in self.team
                if i in self.spies and self.agents[i].betray(self.team, self.leader_id)
            ]
    ...
    def __str__(self):
        ...
        """
        s = "Leader: " + str(self.leader_id) + "\nTeam: "
        sorted_team = sorted(self.team)
        for i in sorted_team:
            s += str(i) + ", "
        s = s[:-2] + "\nVotes for: "
        sorted_votes_for = sorted(self.votes_for)
        for i in sorted_votes_for:
            s += str(i) + ", "
        if self.is_approved():
    ...
    def is_successful(self):
        ...
        return (
            self.is_approved()
            and len(self.fails) < Agent.fails_required[len(self.agents)][self.rnd]
        )
